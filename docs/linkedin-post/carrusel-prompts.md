...
**Re-verificados contra `master` en el commit `79de5ef`** (notificaciones + mejoras visuales).
...
> **Los números NO cambiaron con los commits nuevos, y es una coincidencia — no un descuido.**
> `session-title.js` se **eliminó** y `notify-telegram.js` **entró** en el mismo tramo: −1 +1.
> Si volvés a tocar `hooks/`, recontá. La cuenta buena es:
>
> ```bash
> ls skills/*/SKILL.md | wc -l                    # skills
> ls agents/*.md | wc -l                          # agentes
> ls hooks/*.js | wc -l                           # scripts Node (hooks + statusline + validate)
> ```
>
> Los 15 hooks enganchados = 12 en `settings.json` (`session-bootstrap`, `session-sync`,
> `post-compact-memory`, `notify-desktop`, `notify-telegram`, `clean-arch-guard`, `git-guard`,
> `precommit-validate`, `auto-format`, `session-close-guard`, `subagent-start`, `subagent-index`)
> + 3 por frontmatter de agentes (`atl-only-guard`, `detect-subagent-model`, `judge-output-guard`).

---

## Revisión contra los commits nuevos de `master`

Estos prompts se escribieron sobre `16331d4`. Después entraron cinco commits
(`c045eef`, `e790ccc`, `647f213`, `be46657`, `79de5ef`). Esto es lo que sobrevivió y lo que no.

**Sigue válido, verificado archivo por archivo:** los 7 números de la tabla, los 15 nombres de
skills de las slides 4/5/6, los 5 hooks de la slide 9, y **los 8 modelos por fase de la slide 7**
(`init` haiku · `explore` haiku · `spec` haiku · `design` opus · `tasks` haiku · `apply` sonnet ·
`verify` sonnet · `archive` haiku). Ninguna de esas slides necesita tocarse.

**Lo que sí se corrigió acá:**

| Dónde | Qué pasó en el repo | Corrección aplicada |
|---|---|---|
| **Slide 10** | `79de5ef` reescribió `statusline.js`: ya **no** dibuja `SDD:{change}→{fase}` ni el segmento `‹sesión›`. El primero salió porque una fase no cambia cada 15s (y costaba un `readdirSync` por refresco); el segundo porque el CLI ya muestra el nombre de sesión arriba del input | El mock decía `feat/alta-vales* [sonnet] SDD:alta-vales→design`, un output que el código ya no produce. Ahora dice `⎇ feat/alta-vales* [opus] @dev-orchestrator`, que es el formato real |
| **Slide 3** | Entró `hooks/windows/` (documentado en el README por `be46657`) | Sumada la fila al árbol |
| Tabla de números | `session-title.js` eliminado, `notify-telegram.js` agregado | Los totales no se movieron por pura compensación −1/+1. Documentado arriba para que no se asuma estable |
...
hooks/windows/   bridge de notificaciones
...
"~/erp-facturacion  ⎇ feat/alta-vales*  [opus]  @dev-orchestrator"
...
## SLIDE 9-BIS (opcional) — Notificaciones

No estaba en el plan original de 10 slides: la funcionalidad entró después, en los commits
`c045eef` → `79de5ef`. Es la única cosa del repo que se entiende sin saber nada de Claude Code, así
que como gancho rinde.

**Si la agregás, el carrusel pasa a 11 slides** y hay que recorrer la paginación y los kickers
(`08 / 11`, `09 / 11`, …) de las slides 9 y 10. Si preferís no tocar nada, va como slide 11 al
final, después del CTA — pero pierde, porque después del CTA nadie sigue swipeando.

```
[SISTEMA VISUAL] — acento verde lima #7DBF4A.

Kicker: "09 / 11 · NOTIFICACIONES"
Titular serif grande, dos líneas, la segunda en verde lima:
"Dejás de mirar la terminal.
La terminal te avisa."
Subtítulo sans gris: "Un agente largo no te obliga a quedarte esperando."

Centro: cuatro tarjetas blancas horizontales apiladas, angostas, con borde IZQUIERDO verde lima de
4px. Cada una con el entorno en monoespaciada negra a la izquierda y el mecanismo en sans gris a
la derecha:

Windows / WSL2      toast nativo via BurntToast
macOS               osascript, sin dependencias
Linux escritorio    notify-send
Docker en WSL2      bridge de carpeta + watcher

Debajo, una tarjeta blanca aparte, más ancha y con borde superior verde lima de 3px, con un space
invader verde lima chico a la izquierda, el texto "Telegram" en monoespaciada bold negra, y debajo
en sans gris: "el aviso te llega al celular, no a la máquina donde corre"

Abajo, una franja de fondo verde lima muy suave con texto negro centrado en serif 26px:
"Si falta una dependencia, avisa. No se queda mudo."

Puntos de paginación (once puntos), el décimo activo en verde lima.
```

> **Dato que NO va en la slide:** el bridge de Docker exige levantar un watcher de PowerShell en el
> host y montar un volumen. Es la parte más interesante técnicamente y la peor para un carrusel —
> son tres pasos de setup. Dejalo para el README, que es adonde va a caer quien comente CONFIG.
...
**El copy de arriba sigue siendo correcto tal cual está** — los tres números (29 / 13 / 15), el
reparto de modelos por fase, el review adversarial y la memoria persistente se re-verificaron
contra `79de5ef`. No hay nada que corregir, solo algo que falta.

### Bloque opcional de notificaciones

Si vas con la slide 9-bis, el copy necesita el bullet que la acompañe. Va **después** del de
memoria persistente y antes del cierre:

```
→ Notificaciones que me sacan de la terminal. Windows, macOS, Linux y —lo que más uso— Telegram:
  el agente termina o me pide permiso, y me llega al celular. Dejé de mirar una pantalla esperando
  a que algo pase.
```

Si NO vas con la slide, **no metas el bullet igual**: el carrusel y el texto tienen que contar la
misma historia, y un bullet sin slide que lo respalde es la parte del post que nadie recuerda.
